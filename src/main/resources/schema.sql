...
BEGIN
    IF to_regclass('public.art_requests') IS NOT NULL THEN
        ...
        ALTER TABLE art_requests ALTER COLUMN cover_url TYPE VARCHAR(1000);
        ALTER TABLE art_requests ALTER COLUMN admin_note TYPE VARCHAR(1000);
        ALTER TABLE art_requests ALTER COLUMN rejection_reason TYPE VARCHAR(1000);
    END IF;

    IF to_regclass('public.artworks') IS NOT NULL THEN
        ALTER TABLE artworks ALTER COLUMN description TYPE VARCHAR(1000);
        ALTER TABLE artworks ALTER COLUMN cover_url TYPE VARCHAR(1000);
    END IF;
